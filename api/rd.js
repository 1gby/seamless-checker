export default async function handler(req, res) {
  // Reference environment variable so Vercel compiles this function
  const enabled = process.env.RD_ENABLED;
  ...
  try {
    ...
    if (rdRes.status === 204 || rdRes.status === 202) return res.status(rdRes.status).end();
